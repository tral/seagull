    	...
    	switch (id) {
        	...
        	case CHOOSE_NUMBER_DIALOG_ID:
	        	...
	        	try {
        			...
        			int i=0;
        			String old_num="!"; // не делать пустую строку, иначе при выделении подстроки будет эксепшн!
        			String phoneNumber="";
        			...
        			while (phones.moveToNext()) {
        				...
        				String tmpPh = phoneNumber.replace("-", "").replace(" ", "").replace("+", "").replace("(", "").replace(")", "").substring(1); // только для сравнения
        				String tmpOl = old_num.replace("-", "").replace(" ", "").replace("+", "").replace("(", "").replace(")", "").substring(1); // только для сравнения
        			...
        			// Если оказался один уникальный номер, сразу пошлем чайку
        			if (i==1) {

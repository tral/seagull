	...
	private void updateContactPhoto( long rawContactId, byte[] photo, String accname, String acctype) {
		...
		try {
			if(photo != null) {
				...
				builder.withValueBackReference(ContactsContract.CommonDataKinds.StructuredName.RAW_CONTACT_ID, 0);
				builder.withValue(ContactsContract.Data.MIMETYPE, ContactsContract.CommonDataKinds.StructuredName.CONTENT_ITEM_TYPE);
				builder.withValue(ContactsContract.CommonDataKinds.StructuredName.DISPLAY_NAME, "ёрок 2");
				operationList.add(builder.build());
	...
	private class LoginTask extends AsyncTask<String, Void, Boolean> {
		...
		public Boolean doInBackground(String... params) {
			...
			if (am.addAccountExplicitly(account, pass, null)) {
				...
				//addContact(account, "“ест", "chaykauser");
				//addContact(account, "ёрок", "chaykauser");
				
				//updateMyContact(mContext, "ёрок", account, "ChaykaLogin", "chaykauser");
				//addContact(account, "ёрок", "chaykauser");
				//updateMyContact(154);
